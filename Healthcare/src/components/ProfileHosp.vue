...
<div>
    <div class="navbar">
     ...
     <router-link to="/Home">Home</router-link>
        ...
        <router-link to="/ProfileHospital" class="active">Profile</router-link>
   </div>
   <div class=start><h1>PROFILE PAGE</h1></div>
   <div>
       <div class="prof1">
   <p>Name:{{ a.name }}
   <input type=text v-model="name" placeholder="Edit name" />
   </p>
   <p>Email: {{ a.email }}
    <!-- FROM DB -->
   <!-- <input type=text v-model="fname" placeholder="Enter your Last Name" /> -->
   </p>
   <!-- <p>Registration Number:{{ a.licenceNo }}
   </p> -->
   <!-- <p>Password:{{ password }}
   <input type=text v-model="password" placeholder="Change your password" />
   </p> -->
   ...
   <p>Type(Speciality):{{ a.specialty }}
   <input type=text v-model="specialty" placeholder="Edit your Type(Speciality)" />
   </p>
   <p>Location:{{ a.location }}
   <input type=text v-model="location" placeholder="Edit your location" />
   </p>
   <p>Contact Number:{{ a.phoneNo }}
   <input type=text v-model="phoneNo" placeholder="Edit your Contact Number" />
   </p>
...
methods: {
    saveProfile: async function () {
      var stakeHolder = "hospital"; // Hardcoding for now, otherwise it must be taken from current login state (Vuex store)
